...
function run_example()
    wheat_rows  = [Hedgerow(0.15, 0.73, 3.31, 1.20) for _ in 1:2]
    wheat_inner = [Hedgerow(0.15, 0.73, 2.49, 1.20) for _ in 1:2]
    maize_rows  = [Hedgerow(0.40, 1.24, 1.94, 0.81) for _ in 1:2]

    rows      = vcat(wheat_rows[1:1], wheat_inner, wheat_rows[2:2], maize_rows)
    intercrop = StripIntercrop(rows, 135.0)
    ...
    # Matrix columns: hour, row_idx, f_direct, f_diffuse, f_total
    results = Matrix{Float64}(undef, length(hours) * n_rows, 5)

    row_out = 1
    for hour in hours
        f_dir, f_dif, f_tot = instantaneous_par_interception(intercrop, PAR, lat, doy, hour)
        for i in 1:n_rows
...
function plot_par_interception(results::Matrix{Float64})
    hours  = sort(unique(results[:, 1]))
    n_rows = Int(maximum(results[:, 2]))

    labels     = ["W1(border)", "W2", "W3",  "W4(border)", "M1", "M2"]
    col_names  = ["Direct PAR fraction", "Diffuse PAR fraction", "Total PAR fraction"]
    col_idx    = [3, 4, 5]

    plots = []
    for (col, title) in zip(col_idx, col_names)
        p = plot(title=title, xlabel="Hour (solar)", ylabel="Fraction intercepted",
                 legend=:topright, linewidth=2, marker=:circle, markersize=4)
        for i in 1:n_rows
            mask = results[:, 2] .== Float64(i)
            y    = results[mask, col]
            lbl  = i <= length(labels) ? labels[i] : "Row $i"
            plot!(p, hours, y, label=lbl)
        end
        push!(plots, p)
    end

    fig = plot(plots..., layout=(3, 1), size=(700, 900))
    display(fig)
    return fig
end

fig = plot_par_interception(results)
savefig(fig, "par_interception.png")
